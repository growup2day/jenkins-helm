  ...
  case $variant in
    ...
    "nonprod-backup")
      ...
      if [[ "$ENABLED" == "y" ]]; then
       ...
       print-warn "Make sure you have deactivated the main instance first by running 'DISABLED=y ./deploy.sh csp nonprod'."
    ...
    "prod-backup")
      ...
      if [[ "$ENABLED" == "y" ]]; then
        ...
        print-warn "WARNING: You are trying to deploy and activate the backup Jenkins instance."
        print-warn "Make sure you have deactivated the main instance first by running 'DISABLED=y ./deploy.sh csp prod'."
        print-warn "Enter to continue or Ctrl-C to abort"
        read -r _
      fi
      ;;
    *)
      print-error "Invalid variant specified. See sites/csp/README.md for list of valid variants"
      return 1
  esac
}