...
The CSP site is for provisioning Jenkins instances used by Customer Services Platform teams. The main instances are in the `cs1`
clusters.
...
```sh
./deploy.sh csp nonprod-backup
./deploy.sh csp nonprod
```
...
```sh
DISABLED=y ./deploy.sh csp nonprod
ENABLED=y ./deploy.sh csp nonprod-backup
```
...
```sh
DISABLED=y ./deploy.sh csp prod
ENABLED=y ./deploy.sh csp prod-backup
```
```