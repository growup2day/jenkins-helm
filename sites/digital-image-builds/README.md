# digital-image-builds site

The digital-image-builds site is for provisioning Jenkins instances used by Digital teams. The main instances are in the `cs1`
clusters.
...
```sh
./deploy.sh digital-image-builds nonprod-backup
./deploy.sh digital-image-builds nonprod
```
...
```sh
DISABLED=y ./deploy.sh digital-image-builds nonprod
ENABLED=y ./deploy.sh digital-image-builds nonprod-backup
```
...
```sh
./deploy.sh digital-image-builds prod-backup
./deploy.sh digital-image-builds prod
```

To switch to use the backup instance, run the following commands:

```sh
DISABLED=y ./deploy.sh digital-image-builds prod
ENABLED=y ./deploy.sh digital-image-builds prod-backup
```
```